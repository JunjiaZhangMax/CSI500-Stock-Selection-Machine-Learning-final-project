...
def fetch_industry(retries: int = 3) -> pd.DataFrame:
    """Map stock_code → East Money industry board name.

    Iterates over all industry boards on East Money (~30 boards).
    If a stock appears in multiple boards, keeps the first occurrence.
    """
...
def main():
    ...
    print(">> Fetching industry classification (East Money boards)...")
    industry = fetch_industry()
    industry.to_csv(DATA_DIR / "industry.csv", index=False)
    print(f"   {len(industry)} stocks → {industry['industry'].nunique()} industries → data/industry.csv")
